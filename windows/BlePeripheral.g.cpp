...
EncodableValue PigeonInternalCodecSerializer::ReadValueOfType(
  ...
  switch (type) {
    ...
    case 130: {
        const auto& encodable_enum_arg = ReadValue(stream);
        const int64_t enum_arg_value = encodable_enum_arg.IsNull() ? 0 : encodable_enum_arg.LongValue();
        return encodable_enum_arg.IsNull() ? EncodableValue() : CustomEncodableValue(static_cast<CharacteristicProperties>(enum_arg_value));
      }
    case 131: {
        const auto& encodable_enum_arg = ReadValue(stream);
        const int64_t enum_arg_value = encodable_enum_arg.IsNull() ? 0 : encodable_enum_arg.LongValue();
        return encodable_enum_arg.IsNull() ? EncodableValue() : CustomEncodableValue(static_cast<AttributePermissions>(enum_arg_value));
      }
    case 132: {
...
void PigeonInternalCodecSerializer::WriteValue(
  ...
  if (const CustomEncodableValue* custom_value = std::get_if<CustomEncodableValue>(&value)) {
    ...
    if (custom_value->type() == typeid(CharacteristicProperties)) {
      stream->WriteByte(130);
      WriteValue(EncodableValue(static_cast<int>(std::any_cast<CharacteristicProperties>(*custom_value))), stream);
      return;
    }
    if (custom_value->type() == typeid(AttributePermissions)) {
      stream->WriteByte(131);
      WriteValue(EncodableValue(static_cast<int>(std::any_cast<AttributePermissions>(*custom_value))), stream);
      return;
    }
...
void BlePeripheralChannel::SetUp(
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.ble_peripheral.BlePeripheralChannel.getSubscribedClients" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
