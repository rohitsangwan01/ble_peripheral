    ...
    if (api != nullptr) {
      channel->SetMessageHandler([api](const EncodableValue& message, const flutter::MessageReply<EncodableValue>& reply) {
        try {
          ...
          const auto& encodable_timeout_millis_arg = args.at(2);
          if (encodable_timeout_millis_arg.IsNull()) {
            reply(WrapError("timeout_millis_arg unexpectedly null."));
            return;
          }
          const int64_t timeout_millis_arg = encodable_timeout_millis_arg.LongValue();
          std::optional<FlutterError> output = api->StartAdvertising(services_arg, local_name_arg, timeout_millis_arg);
          if (output.has_value()) {
            reply(WrapError(output.value()));
            return;
          }
