...
interface BlePeripheralChannel {
  ...
  fun startAdvertising(services: List<UUID>, localName: String, timeoutMillis: Long)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val timeoutMillisArg = args[2].let { if (it is Int) it.toLong() else it as Long }
            var wrapped: List<Any?>
            try {
              api.startAdvertising(servicesArg, localNameArg, timeoutMillisArg)
              wrapped = listOf<Any?>(null)
            } catch (exception: Throwable) {
              wrapped = wrapError(exception)
            }
