  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: BlePeripheralChannel?) {
    ...
    if let api = api {
      startAdvertisingChannel.setMessageHandler { message, reply in
        ...
        let timeoutMillisArg = args[2] is Int64 ? args[2] as! Int64 : Int64(args[2] as! Int32)
        do {
          try api.startAdvertising(services: servicesArg, localName: localNameArg, timeoutMillis: timeoutMillisArg)
          reply(wrapResult(nil))
        } catch {
          reply(wrapError(error))
        }
